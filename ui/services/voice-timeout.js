/**
 * Voice timeout service for managing bot disconnection timers
 */
    ...
    if (timeoutMinutes && timeoutMinutes > 0) {
        ...
        if (connection && connection.state.status !== VoiceConnectionStatus.Destroyed) {
            ...
            const timer = setTimeout(async () => {
                ...
                console.log(`[VoiceTimeout] Current session state at timeout:`, {
                    hasSession: guildAudioSessions.has(guildId),
                    nowPlaying: guildAudioSessions.get(guildId)?.nowPlaying ? 'YES' : 'NO',
                    queueLength: guildAudioSessions.get(guildId)?.queue?.length || 0
                });
                console.log(`[VoiceTimeout] Timer details: timeoutMinutes = ${timeoutMinutes}, guildId = ${guildId}`);
                console.log(`[VoiceTimeout] Active timers count: ${activeVoiceTimers.size}`);
                console.log(`[VoiceTimeout] Guild timeouts map:`, Array.from(guildTimeouts.entries()));
                ...
                try {
                    ...
                    if (currentConnection && currentConnection.state.status !== VoiceConnectionStatus.Destroyed) {
                        ...
                        if (currentSession) {
                            ...
                            // COMPREHENSIVE CLEANUP: Use the new unified stop cleanup function
                            try {
                                const { performComprehensiveStopCleanup } = await import('../handlers/common/audio-session.js');
                                await performComprehensiveStopCleanup(guildId, currentSession, 'voice timeout');
                                console.log(`[VoiceTimeout] Comprehensive cleanup completed for guild ${guildId}`);
                            } catch (error) {
                                console.error(`[VoiceTimeout] Error during comprehensive cleanup:`, error.message);
                            }

                            // Clear session data arrays (cleanup function handles the objects)
                            ...
                            // COMPREHENSIVE CLEANUP: Clear all temp files before removing session
                            await cleanupSessionTempFiles(guildId, currentSession);
                            ...
                            // Update playback controls embed to show "Nothing is currently playing"
                            try {
                                await updatePlaybackControlsEmbed(guildId, _djsClient, null);
                                console.log(`[VoiceTimeout] Updated playback controls embed for guild ${guildId} after timeout cleanup`);
                            } catch (error) {
                                console.error(`[VoiceTimeout] Error updating playback controls embed for guild ${guildId}:`, error.message);
                            }

                            console.log(`[VoiceTimeout] Session ${guildId} completely cleaned up and removed due to timeout`);
                        }
                        
                        // Check if this was the last voice connection and stop periodic checks if so
                        const remainingConnections = Array.from(client.guilds.cache.values()).some(guild => {
                            const conn = getVoiceConnection(guild.id);
                            return conn && conn.state.status !== VoiceConnectionStatus.Destroyed;
                        });
                        
                        if (!remainingConnections && global.stopVoiceChecks) {
                            global.stopVoiceChecks();
                        }
                    }
                } catch (error) {
                    ...
                    if (safetySession && safetySession.player && safetySession.player.state && 
                        (safetySession.player.state.status === 'playing' || safetySession.player.state.status === 'buffering')) {
                        console.log(`[VoiceTimeout] ⚠️ SAFETY CHECK FAILED (error case): Music is still playing (${safetySession.player.state.status}), cancelling timeout for guild ${guildId}`);
                        return; // Don't disconnect if music is actually playing
                    }
                    
                    // If we can't check, assume we should disconnect
                    const currentConnection = getVoiceConnection(guildId);
                    if (currentConnection && currentConnection.state.status !== VoiceConnectionStatus.Destroyed) {
                        currentConnection.destroy();
                        console.log(`Bot auto-disconnected from guild ${guildId} due to timeout (error checking members).`);
                        
                        // Clean up session and update UI even in error case
                        const currentSession = guildAudioSessions.get(guildId);
                        if (currentSession) {
                            // Clean up queue message if it exists
                            if (currentSession.queueMessage && currentSession.queueMessage.messageId) {
                                try {
                                    ...
                                    if (channel && channel.isTextBased()) {
                                        await channel.messages.delete(currentSession.queueMessage.messageId);
                                        console.log(`[VoiceTimeout] Deleted queue message ${currentSession.queueMessage.messageId} for guild ${guildId} (error case)`);
                                    }
                                } catch (deleteError) {
                                    console.error(`[VoiceTimeout] Error deleting queue message for guild ${guildId} (error case):`, deleteError.message);
                                }
                            }
                            ...
                            try {
                                ...
                                console.log(`[VoiceTimeout] Cleaned up all processes for guild ${guildId} using unified system (error case)`);
                            } catch (error) {
                                console.error(`[VoiceTimeout] Error cleaning up processes (error case):`, error.message);
                            }
                            ...
                            try {
                                ...
                                console.error(`[VoiceTimeout] Error saving empty queue state for guild ${guildId}:`, saveError.message);
                            }

                            // Update playback controls embed to show "Nothing is currently playing"
                            try {
                                await updatePlaybackControlsEmbed(guildId, _djsClient, null);
                                console.log(`[VoiceTimeout] Updated playback controls embed for guild ${guildId} after timeout cleanup (error case)`);
                            } catch (updateError) {
                                console.error(`[VoiceTimeout] Error updating playback controls embed for guild ${guildId} (error case):`, error.message);
                            }

                            console.log(`[VoiceTimeout] Session ${guildId} completely cleaned up and removed due to timeout (error case)`);
                        }
                    }
                }
